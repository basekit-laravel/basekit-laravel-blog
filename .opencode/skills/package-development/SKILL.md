...
- Publishing tags should be namespaced, e.g. `basekit-laravel-blog-config`, `basekit-laravel-blog-migrations`,
  `basekit-laravel-blog-views`, `basekit-laravel-blog-lang`.
...
- Provide a default config file under `config/basekit-laravel-blog.php`.
- Read values as `config('basekit-laravel-blog.key', $default)` — consumers may override.
...
- Register Blade views with `$this->loadViewsFrom(..., 'basekit-laravel-blog')`; reference them as
  `view('basekit-laravel-blog::name')`.
- Register translations with `$this->loadTranslationsFrom(..., 'basekit-laravel-blog')`; reference
  them as `trans('basekit-laravel-blog::messages.key')`.
