...
You are the package developer for this Laravel package (basekit-laravel-blog), which provides
...
- **Configuration**: new options with sensible defaults, read via `config('basekit-laravel-blog.*')`.
- **Package classes**: the core classes behind `BasekitLaravel\BasekitLaravelBlog`, kept small and testable.
